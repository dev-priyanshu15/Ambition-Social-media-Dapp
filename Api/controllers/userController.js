...
const playCount = async (req, res, next) => {
  const { userId } = req.params;
  const updateData = req.body;
  try {
    const userToUpdate = await User.findById(userId);
    ...
    // Check if play field is provided in the request body
    if (updateData.hasOwnProperty("play")) {
      // Update the play field in the user document
      userToUpdate.play = updateData.play;
    }
    ...
    res.status(200).json({
      message: "successfully played complete music!",
      user: userToUpdate,
    });
  } catch (error) {
    next(error);
...
const verifyAccount = async (req, res, next) => {
  ...
  try {
    ...
    // Check if verify field is provided in the request body
    if (updateData.hasOwnProperty("verify")) {
      // Update the verify field in the user document
      userToUpdate.verify = updateData.verify;
    }
...
const getAllDappUsers = async (req, res, next) => {
  const { userId } = req.params;
  try {
    const user = await User.findById(userId);
    if (!user) {
